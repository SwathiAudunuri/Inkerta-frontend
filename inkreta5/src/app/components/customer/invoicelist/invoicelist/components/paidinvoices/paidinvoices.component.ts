...
import { getcustomerinvoicelist, getcustomerpaidinvoicelist } from '../../../Selectors/invoice.selector';
import { SearchComponent } from '../search/search.component';
import {cloneDeep} from 'lodash';
import { PaidService } from './paid.service';
import { switchMap } from 'rxjs/operators';
import { getcusPaidInvoiceListInitialvalues, getcustomerPaidInvoiceList } from '../../../Actions/paidinvoicelist.action';
import { getCustomerinvoiceAttachments } from '../../../Actions/invoicelistAttachments.action';
import { getcustomerInvoiceListDetailsPdf } from '../../../Actions/invoicelistdetailsPdf.actions';
...
export class PaidinvoicesComponent implements AfterViewInit {
  ...
  fetchRecords(){
    ...
    // this.paid.loadpaid.subscribe(l=>//console.warn(l))


    this.service1 = this.paid.loadpaid.pipe(switchMap(_=>{
      this.store.dispatch(getcustomerPaidInvoiceList({url : Constants.paidInvoiceList}))

      return this.store.select(getcustomerpaidinvoicelist)}
    )) 
    ...
    this.service = this.service1.subscribe((data:any)=>{
      ...
      if(data.paid){
        //console.log(data.paid.invoices.results)
          ...
          if(this.invoicelist){
            this.invoicelistdetails =data.paid.invoices.results
