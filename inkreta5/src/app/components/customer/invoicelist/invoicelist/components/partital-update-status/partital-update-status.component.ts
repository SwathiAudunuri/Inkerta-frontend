...
import { PaidRecords, PaidRecordsnull } from '../../../Actions/paidRecord.actions';
...
import { paidrecordsState, PostupdateStatusState, updateStatusState } from '../../../Selectors/invoice.selector';
...
@Component({
  selector: 'app-partital-update-status',
  templateUrl: './partital-update-status.component.html',
  styleUrls: ['./partital-update-status.component.css']
})
export class PartitalUpdateStatusComponent implements OnInit {
  ref: any
  serive1: any;
  service2: any;
  loading1: any=false;
  ...
  amt_to_pay: any;
  error_amt: any;
  constructor(public dialogRef: MatDialogRef<PartitalUpdateStatusComponent>, private store: Store,
    public dialog: MatDialog,
    private fb: FormBuilder, @Inject(MAT_DIALOG_DATA) public data: any, private unpaid: UnpaidService, private paid: PaidService, private exception: ExceptionServiceload) {
  ...
  ngOnInit(): void {
    ...
    this.store.dispatch(PaidRecordsnull())
    //console.log("------------------------")
    this.getStatus()
    //console.log(this.ref)
  }
  paid_amount(){
    if(this.paid_amt > this.amt_to_pay){
      this.error_amt = true
    }
    else{
      this.paid_due = this.amt_to_pay - this.paid_amt
      this.error_amt = false
    }
    ...
    // }
    // else{
    //   this.pay_status='Partially Paid'
    // }
    
  }
  getStatus() {
    // PaidRecords
    var url1 = Constants.PaidRecords + this.ref
    this.store.dispatch(PaidRecords({url:url1}))
    if(!this.data){
      //console.log("called")
    ...
    this.serive1 = this.store.select(updateStatusState).subscribe((data: any) => {
      ...
      if (data?.result) {
        if (data?.result?.hasError) {
          ...
          this.paid_amt = parseInt(this.total_invoice_value) - parseInt(this.total_pay_amt)
          this.amt_to_pay = parseInt(this.total_invoice_value) - parseInt(this.total_pay_amt)
        }
      }
    })
    ...
    this.service3 = this.store.select(paidrecordsState).subscribe((data:any)=>{
      //console.log(data)
      ...
      if (data?.result) {
        if (data?.result?.hasError) {
          ...
          this.total_pay_amt = 0; 
          for(var i=0;i<this.tabledata.length;i++){
            this.total_pay_amt = this.total_pay_amt + data?.result?.results[i].paid_amount
          }
          this.paid_amt = parseInt(this.total_invoice_value) - parseInt(this.total_pay_amt)
          this.amt_to_pay = parseInt(this.total_invoice_value) - parseInt(this.total_pay_amt)
        }
  ...
  onSubmit() {
    ...
    else if(this.updateForm.value.action === 'Written-Off'){
      var a: any = {
