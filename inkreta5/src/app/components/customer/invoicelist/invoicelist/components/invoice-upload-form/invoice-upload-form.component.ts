...
import { postExternalInvoice } from '../../../Actions/externalInvoiceUpload.actions';
...
export class InvoiceUploadFormComponent implements OnInit {
  ...
  constructor(public datepipe: DatePipe, public dialogRef: MatDialogRef<InvoiceUploadFormComponent>, private dialog: MatDialog, private _formBuilder: FormBuilder, private store: Store, private service: InvoiceUploadService,
  ...
  ngOnInit() {
    ...
    if(this.data?.patnerdata){
      this.invoice_form.patchValue({
        ...
        fromName: this.data?.patnerdata?.companyName,
        fromEmail: this.data?.patnerdata?.email,
        fromMobile: this.data?.patnerdata?.primaryPhoneNumber,
        fromGSTIN: this.data?.patnerdata?.gstin,
        fromaddress: this.data?.patnerdata?.partnerAddress,
      })
    ...
    this.service2 = this.store.select(getpatnerState).subscribe((data:any)=>{
      ...
      if(data.details){
        if(data?.details?.hasError){
          ...
          const address = this.patnerdata[0]?.bno+"," +this.patnerdata[0]?.lgnm+","+this.patnerdata[0]?.bnm+","+this.patnerdata[0]?.loc+","+this.patnerdata[0]?.st+","+this.patnerdata[0]?.pncd
          this.invoice_form.patchValue({
            toName: this.patnerdata[0]?.lgnm,
            toGSTIN: this.patnerdata[0]?.gstin,
            toaddress: address,
            // toEmail: data?.details?.results[0].lgnm,
            // toMobile: data?.details?.results[0].lgnm,
          })
          // this.recipient_partner_id = this.patnerdata[0]?.recipient_partner_id
        }
      }
    })
  ...
  setdefault(){
    const address = this.patnerdata[this.gstindefault]?.bno+"," +this.patnerdata[this.gstindefault]?.lgnm+","+this.patnerdata[this.gstindefault]?.bnm+","+this.patnerdata[this.gstindefault]?.loc+","+this.patnerdata[this.gstindefault]?.st+","+this.patnerdata[this.gstindefault]?.pncd
    this.invoice_form.patchValue({
      toName: this.patnerdata[this.gstindefault]?.lgnm,
      toGSTIN: this.patnerdata[this.gstindefault]?.gstin,
      toaddress: address,
      // toEmail: data?.details?.results[0].lgnm,
      // toMobile: data?.details?.results[0].lgnm,
    })
  ...
  submit() {
    ...
    // const partner_address  = this.invoice_form.value.toName + this.invoice_form.value.toaddress
    var postdata: any = {
      "actionDetails": {
        "action_code": "001",
        "action_name": "Submit",
      ...
      "invoiceDetails": {
        ...
        "recipient_company_name": this.invoice_form.value.fromName,
        "recipient_gstin": this.invoice_form.value.fromGSTIN,
        "recipient_address": this.invoice_form.value.fromaddress,
        ...
        "partner_company_name" : this.invoice_form.value.toName,
        "partner_gstin": this.invoice_form.value.toGSTIN,
        "partner_address":this.invoice_form.value.toaddress,
        "recipient_partner_id":this.recipient_partner_id
      },
      // "mailAddresses":this.invoice_form.value.fromEmail[0],
