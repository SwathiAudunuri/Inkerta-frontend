...
import { QueriedState } from '../../../Selectors/invoice.selector';
import {cloneDeep} from 'lodash';
import { Constants } from 'src/app/constants/constants';
import { switchMap } from 'rxjs/operators';
import { UnpaidService } from '../unpaidinvoices/unpaid.service';
import { QueriesServiceload } from './queries.service';
import { getqueriesInvoiceList, getqueriesListInitialValue } from '../../../Actions/querieslist.action';

@Component({
  selector: 'app-invoice-queries',
  templateUrl: './invoice-queries.component.html',
  styleUrls: ['./invoice-queries.component.css']
})
export class InvoiceQueriesComponent implements OnInit {
  invoice_details: any;
  document_ref_id: any;
  row_index: any;
  ...
  service1: any;
  nameToggle: any=1;
  constructor(private _router: Router,private store:Store,private queries:QueriesServiceload,public unpaid:UnpaidService) { }
  row:any
  id:any
  by:any
  ...
  fetchRecords(){
    ...
    this.service1 = this.queries.loadexception.pipe(switchMap(_=>{
      this.store.dispatch(getqueriesInvoiceList({url : Constants.QueriedInvoiceList}))


      return this.store.select(QueriedState)}
    )) 
   ...
   this.service =  this.service1.subscribe((data:any)=>{
     ...
     if(data.Queried){
      //console.log(data.Queried.invoices.results)
      if(data.Queried.invoices.results.length>0){
        this.invoicelist=data.Queried.invoices.results
        if(this.invoicelist){
          this.invoicelistdetails =data.Queried.invoices.results
