...
import {   getcustomermyinbox } from '../../../Selectors/invoice.selector';
import {cloneDeep} from 'lodash';
import { Constants } from 'src/app/constants/constants';
import { switchMap } from 'rxjs/operators';
import { UnpaidService } from '../unpaidinvoices/unpaid.service';
import { AssignedtomeServiceload } from './assignedtoMe.service';
import { getcustomerMyinbox, getcustomerMyinboxInitialValue } from '../../../Actions/Myinbox.action';
@Component({
  selector: 'app-assignedto-me',
  templateUrl: './assignedto-me.component.html',
  styleUrls: ['./assignedto-me.component.css']
})
export class AssignedtoMeComponent implements OnInit {
  ...
  service: any;
  service1: any;
  constructor(private _router: Router,private store:Store,private assidned:AssignedtomeServiceload,public unpaid:UnpaidService) { }
  row:any
  id:any
  by:any
  ...
  fetchRecords(){
    ...
    this.service1 = this.assidned.loadexception.pipe(switchMap(_=>{
      this.store.dispatch(getcustomerMyinbox({url : Constants.Myinbox}))


      return this.store.select(getcustomermyinbox)}
    )) 
   ...
   this.service =  this.service1.subscribe((data:any)=>{
     ...
     if(data.inbox){
      //console.log(data.inbox.myinbox.results)
      if(data.inbox.myinbox.results.length>0){
        this.invoicelist=data.inbox.myinbox.results
        if(this.invoicelist){
          this.invoicelistdetails =data.inbox.myinbox.results
