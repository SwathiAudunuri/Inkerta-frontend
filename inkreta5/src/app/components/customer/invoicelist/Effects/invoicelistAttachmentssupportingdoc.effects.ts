...
import { getSupportingDocument, getSupportingDocumentSuccess } from "../Actions/invoicelistAttachmentssupportingdoc.action";




@Injectable()
export class CustomerAttachmentsSupportingDocEffects{
    constructor(private action:Actions,private service:CommonService,private store:Store){}

    loadAttachments$=createEffect(()=>
        this.action.pipe(
            ofType(getSupportingDocument),
            // //tap((data)=>//console.log(data)),
            mergeMap((value)=>this.service.getAllRecords({url : value.url}).pipe(
                map(data=>
                    (getSupportingDocumentSuccess({data}))
                ))
