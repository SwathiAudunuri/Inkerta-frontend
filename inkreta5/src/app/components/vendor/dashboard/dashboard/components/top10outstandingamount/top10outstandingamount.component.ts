...
import { gettopten, nulltopten } from '../../../Actions/top10.action';
import { gettoptencustomersState } from '../../../Selectors/top10.selector';
...
//      backgroundColor:'#3c8dbc',#fafabe
  ...
  ngOnInit() {
    ...
    this.store.dispatch(nulltopten())
    this.store.dispatch(gettopten())

    this.getData()
  }
getData(){
  this.store.select(gettoptencustomersState).subscribe((data1:any)=>{
    ...
    if(data1?.result){
      ...
      this.msg="There are no receivable invoices"
      this.loaded=true
    }
