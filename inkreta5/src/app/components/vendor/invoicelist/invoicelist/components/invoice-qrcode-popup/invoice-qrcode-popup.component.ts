...
import { getvendorInvoiceListQrCode } from '../../../Actions/invoicelistQrcode.actions';
import {  getQrinvoicelist } from '../../../Selectors/invoice.selector';
...
export class InvoiceQrcodePopupComponent implements OnInit {
  ...
  getdata(){
    this.service =this.unpaid.loadqrcode.subscribe((ref)=>{
        ...
        this.store.dispatch(getvendorInvoiceListQrCode({doc_refid:ref}))
  ...
  fetchdata(){
    this.store.select(getQrinvoicelist).subscribe((data:any) => {
      //console.log(data)
