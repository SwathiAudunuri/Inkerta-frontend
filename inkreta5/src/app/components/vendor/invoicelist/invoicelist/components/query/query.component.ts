...
// import { getvendorquerylist, queryselector } from '../../../Selectors/query.selector';
import { getvendorquerylist } from '../../../Selectors/invoice.selector';
...
getdata(){
  this.service = this.unpaid.loadquery.subscribe((ref)=>{
    this.store.dispatch(getQueryList({ref:ref}))
...
fetchqueries(){
  ...
  this.store.select(getvendorquerylist).subscribe(state=>{
    console.log(state)
    if(state){
